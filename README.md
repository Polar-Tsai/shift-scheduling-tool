...
**Q: 瀏覽器顯示「無法連線」或 ERR_CONNECTION_REFUSED？**

請檢查以下幾點：

1. **確認伺服器是否成功啟動**
   - 終端機視窗應該顯示：`排班工具伺服器運行在 http://localhost:3000`
   - 如果沒有這個訊息，代表伺服器啟動失敗

2. **終端機視窗必須保持開啟**
   - 執行 `start.bat` 後，**不要關閉**終端機視窗
   - 伺服器會持續運行，直到你按 `Ctrl+C` 停止
...
3. **檢查端口 3000 是否被佔用**
   ...
   ```
   - 如果被佔用，請關閉佔用該端口的程式，或修改 `server.js` 中的 PORT 設定

4. **重新啟動**
   - 在終端機中按 `Ctrl+C` 停止伺服器
   - 重新執行 `start.bat`
   - 等待看到 `排班工具伺服器運行在 http://localhost:3000` 訊息後再開啟瀏覽器
